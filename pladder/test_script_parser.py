...
from pladder.script import ParseError, Call, Quote, Word, Literal, parse
...
def test_open_bracket_closing_brace():
    text = "cmd1 [cmd2}"
    with pytest.raises(ParseError, match="Expected closing bracket, found closing brace"):
        invocation = parse(text)


def test_open_brace_closing_bracket():
    text = "cmd1 {cmd2]"
    with pytest.raises(ParseError, match="Expected closing brace, found closing bracket"):
        invocation = parse(text)
